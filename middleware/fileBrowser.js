...
module.exports = function filebrowser( root, options ) {
	...
	console.log('The stylesheet for fileBrowser: ' + stylesheet);

	module.indexHtml = function( req, done ) {
		...
		if (~path.indexOf('\0')) done(createError(400),'');

		// malicious path
		if ((path + sep).substr(0, root.length) !== root) {
			debug('malicious path "%s"', path);
			done(createError(403),'');
			return;
		}
		...
		fs.stat(path, function(err, stat) {
			...
			if (!stat.isDirectory()) { done(null, ''); return; };
			...
			fs.readdir( path, function(err, files) {
				...
				// not acceptable
				if (!type) { done(createError(406),''); return;};

				getHtml( req, files, originalDir, showUp, icons, path, view, template, stylesheet, function(err, resHtml) {
					done(err, resHtml);
				});
			});
		});
	...
	function getHtml(req, files, dir, showUp, icons, path, view, template, stylesheet, done) {
		console.log('Inside getHtml');
		fs.readFile(template, 'utf8', function(err, str){
			console.log('Inside getHtml: fs.readFile Template');
			if (err) done(err, '');
			fs.readFile(stylesheet, 'utf8', function(err, style){
				console.log('Inside getHtml: fs.readFile Stylesheet: ' + stylesheet);
				console.log('Inside getHtml: files: ' + files.toString());
				if (err) {
					console.log('There is an error: ' + err.code);
					return  done(err, '');
				}
				stat(path, files, function(err, stats){
					console.log('Inside getHtml: stat: ');
					if (err) done(err, '');
